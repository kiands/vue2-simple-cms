        ...
        >
          ...
          <v-expansion-panel-content>
            ...
            <div style="margin-top: 12px; display: flex; flex-direction: row; justify-content: center;">
              <v-btn style="margin-right: 24px" @click="editProgram(carousel)">EDIT</v-btn>
              <v-btn @click="deleteProgram(program.image_id)">DELETE</v-btn>
            </div>
            <div v-if="edit_program" style="width: 100%; display: flex; flex-direction: column;">
              ...
              <ImageChangeComponent :action="'edit'" :api="'cms/programs/'" :code="1" :id="program.image_id" @Submission="handleSubmission"/>
  ...
  export default {
    ...
    data() {
      return {
        add_program: false,
        edit_program: false,
        programs: [{ image_id: 1, source: "http://127.0.0.1", link: "http://127.0.0.1" }]
      };
    },
    methods: {
      editProgram(program) {
